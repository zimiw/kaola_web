	...
	$scope.createMenuDom = function(content) {
		...
		for (var i = 0; i < length; i++) {
			var temp = '<div id="' + 'meun_' + content[i].id + '" class="col-sm-10 col-md-6 col-lg-3"><div class="thumbnail"><div class="img-wrap">' 
					+ '<img src="' + content[i].imgUrl + '" alt="farrir" class="img-rounded"></div><div class="caption">' + '<div class="first-line"><b class="title">' 
					+ content[i].name + '</b>' + '<a href="javascript:#" role="button" aria-expanded="true"><span class="glyphicon glyphicon-heart-empty"></span></a>' 
					+ '<div><em>' + content[i].price + '</em><span>w元</span></div></div><div class="second-line">' + '<div>月售<strong>' + content[i].monthCount 
					+ '</strong>份</div></div>' + '<div class="third-line"><a class="btn btn-primary" type="button" ng-click="addToOrder('+ content[i].id + ')">来一份<span class="badge">0</span></a>' 
